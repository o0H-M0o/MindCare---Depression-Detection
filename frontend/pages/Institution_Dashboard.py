...
def _render_user_analysis(user: dict, user_df: pd.DataFrame) -> None:
    # --- Personal Dashboard style (tabs + global day range filter) ---
    ...
    def _label_severity(score: float) -> str:
        """Map average BDI score to standard categories for staff view."""
        if pd.isna(score):
            return "N/A"
        if score < 10:
            return "Minimal"
    ...
    last30_session_df = session_df[session_df["datetime"] >= cutoff_30].copy() if session_df is not None and not session_df.empty else pd.DataFrame()
    last30_daily_df = daily_df[daily_df["datetime"] >= cutoff_30].copy() if daily_df is not None and not daily_df.empty else pd.DataFrame()
    last30_avg_score = float(pd.to_numeric(last30_session_df.get("bdi_total_score"), errors="coerce").dropna().mean()) if not last30_session_df.empty else float("nan")
    last30_distress_level = _label_severity(last30_avg_score)
    ...
    with overview_tab:
        ...
        with col_left:
            ...
            if global_daily_df is None or global_daily_df.empty:
                ...
                st.plotly_chart(fig_bdi_overview, width="stretch", key=f"pd_bdi_overview_{user['id']}_{global_chart_view}")
        ...
        with col_right:
            ...
            if top_symptoms_df is None or top_symptoms_df.empty:
                ...
                st.plotly_chart(fig_symptoms_overview, width="stretch", key=f"pd_symptoms_overview_{user['id']}_{global_chart_view}")
    ...
    with trends_tab:
        ...
        with col1:
            ...
            if w is None or w.empty:
                ...
                st.plotly_chart(fig_bdi, width="stretch", key=f"pd_weekly_distress_{user['id']}_{global_chart_view}")
        ...
        with col2:
            ...
            if w_sent.empty:
                ...
                st.plotly_chart(fig_s, width="stretch", key=f"pd_moodmix_{user['id']}_{global_chart_view}")
    ...
    with patterns_tab:
        ...
        with col1:
            ...
            if global_daily_df is None or global_daily_df.empty:
                ...
                st.plotly_chart(fig_dow, width="stretch", key=f"pd_dow_{user['id']}_{global_chart_view}")
        ...
        with col2:
            ...
            if global_session_df is None or global_session_df.empty:
                ...
                st.plotly_chart(fig_tod, width="stretch", key=f"pd_tod_{user['id']}_{global_chart_view}")
    ...
    with support_tab:
            ...
            if export_df is None or export_df.empty:
                ...
                if csv_bytes:
                    st.download_button(
                        label="📄 Download CSV",
                        data=csv_bytes,
                        file_name=f"institution_personal_{user.get('name','user').replace(' ', '_')}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
                        mime="text/csv",
                        key=f"pd_csv_{user['id']}_{global_chart_view}",
                        width="stretch",
                    )
        ...
        with export_col2:
            try:
                collected_figs = []
                # Rebuild figures for PDF from the same data slices
                ...
                # Overview: Key symptoms to monitor
                if top_symptoms_df is not None and not top_symptoms_df.empty:
                    try:
                ...
                try:
                    if global_session_df is not None and not global_session_df.empty:
                        ...
                        dfp_time_export["tod"] = dfp_time_export["hour"].apply(
                            lambda h: _bucket(int(h)) if not pd.isna(h) else "Unknown"
                        )
                ...
                if collected_figs:
                    ...
                    selected_period_level_pdf = str(selected_period_distress_level)
                    ...
                    metrics = {
                        ...
                        f"{global_chart_view} distress level": str(selected_period_level_pdf),
                    ...
                    # Institution reports should not include end-user guidance text.
                    ...
                    if pdf_bytes:
                        st.download_button(
                            ...
                            data=pdf_bytes,
                            file_name=f"institution_personal_{user.get('name','user').replace(' ', '_')}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf",
                            mime="application/pdf",
                            key=f"pd_pdf_{user['id']}_{global_chart_view}",
                            width="stretch",
                        )
                else:
